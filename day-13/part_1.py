...
def horizontal(grid):
    for i, _ in enumerate(grid[:-1]):
        if all(row_above == row_below for row_above, row_below in zip(grid[i::-1], grid[i + 1:])):
            return i + 1
    return None
...
def vertical(grid):
    for i, _ in enumerate(grid[0][:-1]):
        if all(all(left == right for left, right in zip(row[i::-1], row[i + 1:])) for row in grid):
            return i + 1
    return None
